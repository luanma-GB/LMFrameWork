...
public class CcImageCache {
	...
	private static final LruCache<String, Bitmap> bitmapCache = new LruCache<String, Bitmap>(
		...
		protected void entryRemoved(boolean evicted, String key,
			...
			CcLog.d(CcImageCache.class, "LruCache:移除了" + key);
		}
	...
	public static void addBitmapToCache(String key, Bitmap bitmap) {
		try {
			...
			if (getBitmapFromCache(key) == null && bitmap != null) {
				...
				// if(D) Log.d(TAG,
				// "测试存入缓存是否成功:"+key+","+getBitmapFromCache(key));
			}
			// 表示下载中的缓存清除
	...
	public static String getCacheKey(String url, int width, int height, int type) {
		return CcMd5.MD5(new StringBuilder(url.length() + 12).append("#W")
				.append(width).append("#H").append(height).append("#T")
				.append(type).append(url).toString());
	}
	...
	public static void addToRunRunnableCache(String key, Runnable runnable) {
		try {
			lock.lock();
			if (CcStrUtil.isEmpty(key) || runnable == null) {
				return;
			}
