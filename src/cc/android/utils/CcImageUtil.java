...
public class CcImageUtil {
	...
	public static Bitmap getBitmapFormURL(String imageUrl, int type,
		...
		try {
			...
			CcLog.d(CcImageUtil.class, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					CcLog.d(CcImageUtil.class, "Bitmap释放" + bitmap.toString());
					bitmap.recycle();
				}
			} catch (Exception e) {
			}
	...
	public static String getHashCode(Bitmap bitmap) {
		...
		// 第三步，计算平均值。
		// 计算所有64个像素的灰度平均值。
		int avgPixel = CcMathUtil.average(pixels);
		...
		for (int i = 0; i < comps.length; i += 4) {
			...
			hashCode.append(CcMathUtil.binaryToHex(result));
		}
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int[][] pxMatrix = CcMathUtil.arrayToMatrix(pixels, width, height);
		double[][] doublePxMatrix = CcMathUtil.intToDoubleMatrix(pxMatrix);

		// 计算DCT,已经变成8*8了
		double[][] dtc = FDCT.fDctTransform(doublePxMatrix);

		// 计算平均值。
		double[] dctResult = CcMathUtil.matrixToArray(dtc);
		int avgPixel = CcMathUtil.average(dctResult);
